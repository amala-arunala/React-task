...
const MainPage = () => {
     ...
     const btnHandler = () => {
          ...
          const filterBuses = data.filter(
               (bus) => bus.fromLocation.toLowerCase() === fromLocation.toLowerCase() 
     ...
     const onButton = (
               busList.length ? ( busList.map((list, index) => (
                     <BusDetails key={index}
                         bus = {list.busName}
  ...
  return (
    <div>
        ...
        <label htmlFor='from-addr' style={{margin : "5px"}}>From</label>
        <select id="from-addr" value={fromLocation} onChange={(e) => setFromLocation(e.target.value)}>
        ...
        <label htmlFor='to-addr' style={{paddingRight : "10px"}}>To</label>
        <select id='to-addr' value={toLocation} onChange={(e) => setToLocation(e.target.value)}>
        <option value="">-- choose --</option>
        ...
        <div className='search'>
           <button className='search-btn' onClick={btnHandler}>Search Buses</button>
        </div>
